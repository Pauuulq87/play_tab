  ...
  return (
    <aside className="w-80 flex-shrink-0 flex flex-col border-l border-steel dark:border-gray-700 h-screen bg-charcoal dark:bg-dark-surface text-paper dark:text-gray-300 transition-colors duration-200">
      ...
      <div className="h-16 border-b border-steel/50 dark:border-gray-700 flex items-center justify-between px-4 shrink-0 bg-charcoal dark:bg-dark-surface">
        <Sidebar size={18} className="text-steel-light dark:text-gray-500 hover:text-white cursor-pointer" />
        <span className="text-xs font-bold tracking-[0.2em] text-white dark:text-gray-300 uppercase">Open Tabs</span>
      </div>
      ...
      <div className="p-4 border-b border-steel/30 dark:border-gray-700 bg-charcoal dark:bg-dark-surface">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 cursor-pointer group">
            <span className="font-serif text-sm font-medium text-white group-hover:text-steel-light transition-colors">Window 1</span>
            <ChevronDown size={14} className="text-steel-light" />
          </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-2">
        {OPEN_TABS.map((tab) => (
          <div 
            key={tab.id}
            className="
              group relative bg-steel/20 dark:bg-black/20 border border-transparent hover:border-l-4 hover:border-l-white hover:bg-steel/30 dark:hover:bg-black/40
              px-3 py-3 transition-all cursor-pointer min-h-[40px] flex items-center
            "
          >
            <span className="text-xs font-sans text-gray-300 group-hover:text-white line-clamp-2 leading-relaxed">
              {tab.title}
            </span>
            ...
            <div className="absolute right-2 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-2">
               <X size={12} className="text-white opacity-50 hover:opacity-100" />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="p-2 text-center border-t border-steel/30 dark:border-gray-700">
          <span className="text-[10px] text-steel-light font-mono">16 Tabs Open</span>
      </div>
    </aside>
  );
};
